function dentario_googlemap_init(dom_obj, coords) {
  "use strict";
  if (typeof DENTARIO_STORAGE["googlemap_init_obj"] == "undefined")
    dentario_googlemap_init_styles();
...
function dentario_googlemap_create(id) {
  ...
  // Create map
  DENTARIO_STORAGE["googlemap_init_obj"][id].map = new google.maps.Map(
    DENTARIO_STORAGE["googlemap_init_obj"][id].dom,
    DENTARIO_STORAGE["googlemap_init_obj"][id].opt
  );
  ...
  jQuery(window).resize(function() {
    if (DENTARIO_STORAGE["googlemap_init_obj"][id].map)
      DENTARIO_STORAGE["googlemap_init_obj"][id].map.setCenter(
        DENTARIO_STORAGE["googlemap_init_obj"][id].opt.center
      );
  });
}
...
function dentario_googlemap_add_markers(id) {
  ...
  for (var i in DENTARIO_STORAGE["googlemap_init_obj"][id].markers) {
    ...
    if (DENTARIO_STORAGE["googlemap_init_obj"][id].markers[i].latlng == "") {
      if (DENTARIO_STORAGE["googlemap_init_obj"][id].geocoder_request !== false)
        continue;

      if (DENTARIO_STORAGE["googlemap_init_obj"].geocoder == "")
        DENTARIO_STORAGE[
          "googlemap_init_obj"
        ].geocoder = new google.maps.Geocoder();
      DENTARIO_STORAGE["googlemap_init_obj"][id].geocoder_request = i;
      DENTARIO_STORAGE["googlemap_init_obj"].geocoder.geocode(
        {
          address: DENTARIO_STORAGE["googlemap_init_obj"][id].markers[i].address
        },
        function(results, status) {
          "use strict";
          if (status == google.maps.GeocoderStatus.OK) {
            var idx =
              DENTARIO_STORAGE["googlemap_init_obj"][id].geocoder_request;
            if (
              results[0].geometry.location.lat &&
              results[0].geometry.location.lng
            ) {
              DENTARIO_STORAGE["googlemap_init_obj"][id].markers[idx].latlng =
                "" +
                results[0].geometry.location.lat() +
                "," +
                results[0].geometry.location.lng();
            } else {
              DENTARIO_STORAGE["googlemap_init_obj"][id].markers[
                idx
      ...
      var latlngStr = DENTARIO_STORAGE["googlemap_init_obj"][id].markers[
        i
      ].latlng.split(",");
      var markerInit = {
        ...
        clickable:
          DENTARIO_STORAGE["googlemap_init_obj"][id].markers[i].description !=
          ""
      };
      if (DENTARIO_STORAGE["googlemap_init_obj"][id].markers[i].point)
        markerInit.icon =
          DENTARIO_STORAGE["googlemap_init_obj"][id].markers[i].point;
      if (DENTARIO_STORAGE["googlemap_init_obj"][id].markers[i].title)
        markerInit.title =
          DENTARIO_STORAGE["googlemap_init_obj"][id].markers[i].title;
      DENTARIO_STORAGE["googlemap_init_obj"][id].markers[
        i
      ].marker = new google.maps.Marker(markerInit);
        ...
        google.maps.event.addListener(
          ...
          function(e) {
            ...
            for (var i in DENTARIO_STORAGE["googlemap_init_obj"][id].markers) {
              if (
                latlng ==
                DENTARIO_STORAGE["googlemap_init_obj"][id].markers[i].latlng
              ) {
                DENTARIO_STORAGE["googlemap_init_obj"][id].markers[
                  i
                ].infowindow.open(
...
function dentario_googlemap_init_styles() {
  ...
  if (window.dentario_theme_googlemap_styles !== undefined)
    DENTARIO_STORAGE["googlemap_styles"] = dentario_theme_googlemap_styles(
      DENTARIO_STORAGE["googlemap_styles"]
    );
}

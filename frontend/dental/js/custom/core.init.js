...
function dentario_init_actions() {
  ...
  if (
    DENTARIO_STORAGE["vc_edit_mode"] &&
    jQuery(".vc_empty-placeholder").length == 0 &&
    DENTARIO_STORAGE["theme_init_counter"]++ < 30
  ) {
...
function dentario_ready_actions() {
  ...
  if (jQuery("ul#menu_side").length > 0) {
    ...
    dentario_show_current_menu_item(
  ...
  dentario_init_sfmenu(
    '.menu_main_nav_area ul#menu_main, ul[id^="menu_user"], ul#menu_side, body:not(.woocommerce) .widget_area:not(.footer_wrap) .widget_product_categories ul.product-categories'
  );
  ...
  jQuery(
    '#toc, ul#menu_main li, ul[id^="menu_user"] li, ul#menu_side li, ul#menu_footer li, ul#menu_pushy li'
  ...
  jQuery("#viewmore_link").on("click", function(e) {
    "use strict";
    if (
      !DENTARIO_STORAGE["viewmore_busy"] &&
  ...
  jQuery(".bookmarks_add").on("click", function(e) {
    ...
    if (!exists) {
      var message_popup = dentario_message_dialog(
        '<label for="bookmark_title">' +
          DENTARIO_STORAGE["strings"]["bookmark_title"] +
          '</label><br><input type="text" id="bookmark_title" name="bookmark_title" value="' +
          title +
          '">',
        DENTARIO_STORAGE["strings"]["bookmark_add"],
        null,
        function(btn, popup) {
          "use strict";
          if (btn != 1) return;
          ...
          jQuery(".bookmarks_list").append(
            '<li><a href="' +
              ...
              DENTARIO_STORAGE["strings"]["bookmark_del"] +
              '"></span></a></li>'
          );
          jQuery.cookie("dentario_bookmarks", JSON.stringify(list), {
            expires: 365,
            path: "/"
          });
          setTimeout(function() {
            dentario_message_success(
              DENTARIO_STORAGE["strings"]["bookmark_added"],
              DENTARIO_STORAGE["strings"]["bookmark_add"]
            );
          }, DENTARIO_STORAGE["message_timeout"] / 4);
        }
      );
    } else dentario_message_warning(DENTARIO_STORAGE["strings"]["bookmark_exists"], DENTARIO_STORAGE["strings"]["bookmark_add"]);
    e.preventDefault();
    return false;
  });
  ...
  if (window.dentario_init_hidden_elements)
    dentario_init_hidden_elements(jQuery("body").eq(0));
} //end ready
...
function dentario_scroll_actions() {
  ...
  if (DENTARIO_STORAGE["top_panel_height"] < 1) {
    DENTARIO_STORAGE["top_panel_height"] = Math.max(
      0,
  ...
  if (
    ...
    if (jQuery(".top_panel_below .slider_wrap").length > 0) {
      ...
      if (slider_height < 10) {
        slider_height = jQuery(".slider_wrap").hasClass(".slider_fullscreen")
          ...
          : DENTARIO_STORAGE["slider_height"];
      }
    }
    if (scroll_offset <= slider_height + DENTARIO_STORAGE["top_panel_height"]) {
      ...
      scroll_offset >
      slider_height + DENTARIO_STORAGE["top_panel_height"]
    ) {
      if (
        ...
        jQuery(".top_panel_fixed_wrap").height(
          DENTARIO_STORAGE["top_panel_height"]
        );
  ...
  if (jQuery(".sidebar_outer").length > 0) {
    if (DENTARIO_STORAGE["side_panel_height"] == 0)
      DENTARIO_STORAGE["side_panel_height"] =
    ...
    if (
      ...
      DENTARIO_STORAGE["side_panel_height"] + 100
    ) {
  ...
  jQuery("#toc .toc_item").each(function() {
    ...
    if (
      ...
      scroll_offset + DENTARIO_STORAGE["top_panel_height"] < off_next
    )
...
function dentario_resize_video_background() {
  ...
  if (
    DENTARIO_STORAGE["media_elements_enabled"] &&
    ...
    .each(function() {
      ...
      jQuery(this)
        ...
        .superfish({
          ...
          speed: DENTARIO_STORAGE["css_animation"]
            ? 500
            : DENTARIO_STORAGE["menu_slider"]
              ? 300
              : 200,
          speedOut: DENTARIO_STORAGE["css_animation"]
            ? 500
            : DENTARIO_STORAGE["menu_slider"]
              ? 300
              : 200,
          ...
          onBeforeShow: function(ul) {
            ...
            if (DENTARIO_STORAGE["css_animation"]) {
              jQuery(this).removeClass(
                "animated fast " + DENTARIO_STORAGE["menu_animation_out"]
              );
              jQuery(this).addClass(
                "animated fast " + DENTARIO_STORAGE["menu_animation_in"]
              );
            }
          },
...
function dentario_build_page_toc() {
  ...
  if (
    toc_count >
    (DENTARIO_STORAGE["toc_menu_home"] ? 1 : 0) +
      (DENTARIO_STORAGE["toc_menu_top"] ? 1 : 0)
  ) {
    if (jQuery("#toc").length > 0) jQuery("#toc .toc_inner").html(toc);
    else
      jQuery("body").append(
        '<div id="toc" class="toc_' +
          DENTARIO_STORAGE["toc_menu"] +
          '"><div class="toc_inner">' +
          toc +
          "</div></div>"
      );
...
function dentario_init_isotope() {
  ...
  jQuery(".isotope_wrap:not(.inited)").each(function() {
    "use strict";
    all_images_complete =
      all_images_complete && dentario_check_images_complete(jQuery(this));
  });
...
function dentario_init_appended_isotope(posts_container, filters) {
  ...
  if (
    !dentario_check_images_complete(posts_container) &&
    DENTARIO_STORAGE["isotope_init_counter"]++ < 30
  ) {
    setTimeout(function() {
      dentario_init_appended_isotope(posts_container, filters);
    }, 200);
    return;
  }
  // Add filters
...
function dentario_init_post_formats() {
  ...
  // Call theme specific action (if exists)
  if (window.dentario_theme_init_post_formats)
    dentario_theme_init_post_formats();
          ...
          .done(function(response) {
            ...
            if (rez.error === "") {
              ...
              dentario_message_warning(
                DENTARIO_STORAGE["strings"]["error_like"]
              );
            }
          });
        e.preventDefault();
...
function dentario_init_media_elements(cont) {
  ...
  if (
    ...
    if (window.mejs) {
      ...
      setTimeout(function() {
        dentario_init_media_elements(cont);
      }, 400);
  }
}
...
function dentario_show_system_message() {
  ...
  if (
    ...
    DENTARIO_STORAGE["system_message"]["message"]
  ) {
    if (DENTARIO_STORAGE["system_message"]["status"] == "success")
      dentario_message_success(
        DENTARIO_STORAGE["system_message"]["message"],
        DENTARIO_STORAGE["system_message"]["header"]
      );
      ...
      DENTARIO_STORAGE["system_message"]["status"] == "error" ||
      DENTARIO_STORAGE["system_message"]["status"] == "warning"
    )
      dentario_message_warning(
        DENTARIO_STORAGE["system_message"]["message"],
        DENTARIO_STORAGE["system_message"]["header"]
...
function dentario_toggle_popup(popup) {
  "use strict";
  if (popup.css("display") != "none") dentario_hide_popup(popup);
  else dentario_show_popup(popup);
}
...
function dentario_show_popup(popup) {
  ...
  if (popup.css("display") == "none") {
    if (DENTARIO_STORAGE["css_animation"])
      popup
        .show()
        .removeClass("animated fast " + DENTARIO_STORAGE["menu_animation_out"])
        .addClass("animated fast " + DENTARIO_STORAGE["menu_animation_in"]);
...
function dentario_hide_popup(popup) {
  ...
  if (popup.css("display") != "none") {
    if (DENTARIO_STORAGE["css_animation"])
      popup
        .removeClass("animated fast " + DENTARIO_STORAGE["menu_animation_in"])
        .addClass("animated fast " + DENTARIO_STORAGE["menu_animation_out"])
        .delay(500)
        .hide();
    else popup.fadeOut();
  }
  ...
  var error = dentario_form_validate(form, {
    ...
    rules: [
      ...
      },
      {
        ...
        max_length: {
          value: DENTARIO_STORAGE["comments_maxlength"],
    ...
    rules: [
      {
        ...
        min_length: {
          value: 1,
          message: DENTARIO_STORAGE["strings"]["login_empty"]
        },
        max_length: {
          value: 60,
          message: DENTARIO_STORAGE["strings"]["login_long"]
        }
      },
      {
        field: "pwd",
        min_length: {
          value: 4,
          message: DENTARIO_STORAGE["strings"]["password_empty"]
        },
        max_length: {
          value: 30,
          message: DENTARIO_STORAGE["strings"]["password_long"]
        }
      }
    ]
  });
      ...
      .done(function(response) {
        ...
        if (rez.error === "") {
          result_box
            ...
            .html(DENTARIO_STORAGE["strings"]["login_success"]);
          setTimeout(function() {
            location.reload();
          }, 3000);
        } else {
    ...
    rules: [
      ...
      },
      {
        ...
        equal_to: {
          ...
          message: DENTARIO_STORAGE["strings"]["password_not_equal"]
        }
      }
    ]
  });
    ...
    jQuery
      ...
      .done(function(response) {
        ...
        if (rez.error === "") {
          result_box
            .addClass("sc_infobox sc_infobox_style_success")
            .html(DENTARIO_STORAGE["strings"]["registration_success"]);
          setTimeout(function() {
          ...
          result_box
            .addClass("sc_infobox sc_infobox_style_error")
            .html(
              DENTARIO_STORAGE["strings"]["registration_failed"] +
                " " +
                rez.error
            );
        }

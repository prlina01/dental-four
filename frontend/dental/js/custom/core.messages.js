...
jQuery(document).ready(function() {
  ...
  jQuery("body").on(
    "click",
    "#dentario_modal_bg,.dentario_message .dentario_message_close",
    function(e) {
      "use strict";
...
function dentario_message(opt) {
  ...
  var delay =
    opt.delay != undefined ? opt.delay : DENTARIO_STORAGE["message_timeout"];
  ...
  var html =
    '<div class="dentario_message dentario_message_' +
    type +
    (buttons.length > 0 ? " dentario_message_dialog" : "") +
    '">' +
    '<span class="dentario_message_close iconadmin-cancel icon-cancel"></span>' +
    (icon
      ? '<span class="dentario_message_icon iconadmin-' +
        icon +
        " icon-" +
        icon +
        '"></span>'
  ...
  jQuery("body .dentario_message").animate(
    {
      top:
        top +
        Math.round(
          (jQuery(window).height() - jQuery(".dentario_message").height()) / 2
        ),
      opacity: 1
    },
    {
...
function dentario_message_destroy() {
  ...
  jQuery(".dentario_message").animate({
    top: top - jQuery(".dentario_message").height(),
    opacity: 0
  });
  setTimeout(function() {
    jQuery("#dentario_modal_bg").remove();
